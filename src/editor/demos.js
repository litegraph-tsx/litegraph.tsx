...
function multiConnection() {
  ...
  const node_const_A = LiteGraph.createNode('basic/const');
  node_const_A.pos = [200, 200];
  graph.add(node_const_A);
  node_const_A.setValue(4.5);

  const node_const_B = LiteGraph.createNode('basic/const');
  node_const_B.pos = [200, 300];
  graph.add(node_const_B);
  node_const_B.setValue(10);

  const node_math = LiteGraph.createNode('math/operation');
...
function CopyPasteWithConnectionToUnselectedOutputTest() {
  // number
  const nodeConstA = LiteGraph.createNode('basic/const');
  nodeConstA.pos = [200, 200];
  graph.add(nodeConstA);
  nodeConstA.setValue(4.5);

  // number
  const nodeConstB = LiteGraph.createNode('basic/const');
  nodeConstB.pos = [200, 300];
  graph.add(nodeConstB);
  nodeConstB.setValue(10);

  // math
  const nodeMath = LiteGraph.createNode('math/operation');
...
function sortTest() {
  const rand = LiteGraph.createNode('math/rand', null, { pos: [10, 100] });
  graph.add(rand);

  const nodes = [];
  for (var i = 4; i >= 1; i--) {
    const n = LiteGraph.createNode('basic/watch', null, { pos: [i * 120, 100] });
    graph.add(n);
    nodes[i - 1] = n;
  }
...
function benchmark() {
  ...
  for (var i = 0; i < num_nodes; i++) {
    const n = LiteGraph.createNode('basic/watch', null, { pos: [(2000 * Math.random()) | 0, (2000 * Math.random()) | 0] });
    graph.add(n);
    nodes.push(n);
  }
