...
import { LGraphStyles, node_colors } from './styles';
import {
  clamp,
...
export class LGraphCanvas {
  constructor(canvas, graph, options) {
    ...
    this.default_link_color = LGraphStyles.LINK_COLOR;
    ...
    this.links_render_mode = LGraphStyles.SPLINE_LINK;
          ...
          if (this.connecting_output) {
            ...
            if (this.isOverNodeBox(node, e.canvasX, e.canvasY)) {
              ...
              if (slot != -1 && node.outputs[slot]) {
                ...
                if (isValidConnection(this.connecting_input.type, slot_type)) {
    ...
    if (e.which == 1) {
      ...
      if (this.dragging_rectangle) {
        ...
        if (
          node
                        && e.click_time < 300
                        && isInsideRectangle(e.canvasX, e.canvasY, node.pos[0], node.pos[1] - LGraphStyles.NODE_TITLE_HEIGHT, LGraphStyles.NODE_TITLE_HEIGHT, LGraphStyles.NODE_TITLE_HEIGHT)
        ) {
          node.collapse();
        }
  ...
  pasteFromClipboard(isConnectUnselected = false) {
    ...
    for (var i = 0; i < clipboard_info.nodes.length; ++i) {
      ...
      const node = createNode(node_data.type);
      if (node) {
  ...
  checkDropItem(e) {
    if (e.dataTransfer.files.length) {
      ...
      const nodetype = LGraphNodeRegistry.node_types_by_file_extension[ext];
      if (nodetype) {
        this.graph.beforeChange();
        const node = createNode(nodetype.type);
      ...
      if (this.connecting_pos != null) {
        ...
        switch (connType) {
          case LGraphEvents.EVENT:
            ...
            break;
          default:
            link_color = LGraphStyles.CONNECTING_LINK_COLOR;
        }
        ...
        } else if (connShape === LGraphStyles.ARROW_SHAPE) {
        ...
        if (this._highlight_input) {
          ctx.beginPath();
          var { shape } = this._highlight_input_slot;
          if (shape === LGraphStyles.ARROW_SHAPE) {
  ...
  drawNode(node, ctx) {
    ...
    if (this.render_shadows && !low_quality) {
      ctx.shadowColor = LGraphStyles.DEFAULT_SHADOW_COLOR;
    ...
    const shape = node._shape || LGraphStyles.BOX_SHAPE;
    ...
    if (node.clip_area) {
      ...
      ctx.save();
      ctx.beginPath();
      if (shape == LGraphStyles.BOX_SHAPE) {
        ctx.rect(0, 0, size[0], size[1]);
      } else if (shape == LGraphStyles.ROUND_SHAPE) {
        ctx.roundRect(0, 0, size[0], size[1], [10]);
      } else if (shape == LGraphStyles.CIRCLE_SHAPE) {
        ctx.arc(
          size[0] * 0.5,
          size[1] * 0.5,
          size[0] * 0.5,
    ...
    if (!node.flags.collapsed) {
      ...
      if (node.inputs) {
        for (var i = 0; i < node.inputs.length; i++) {
          ...
          if (this.connecting_output && !isValidConnection(slot.type, out_slot.type)) {
          ...
          if (slot_type == 'array') {
            slot_shape = LGraphStyles.GRID_SHAPE; // place in addInput? addOutput instead?
          }
          ...
          if (
          ...
          } else if (slot_shape === LGraphStyles.ARROW_SHAPE) {
            ctx.moveTo(pos[0] + 8, pos[1] + 0.5);
            ctx.lineTo(pos[0] - 4, pos[1] + 6 + 0.5);
            ctx.lineTo(pos[0] - 4, pos[1] - 6 + 0.5);
            ctx.closePath();
          } else if (slot_shape === LGraphStyles.GRID_SHAPE) {
            ...
            doStroke = false;
          } else if (low_quality) ctx.rect(pos[0] - 4, pos[1] - 4, 8, 8); // faster
      ...
      if (node.outputs) {
        for (var i = 0; i < node.outputs.length; i++) {
          ...
          // change opacity of incompatible slots when dragging a connection
          if (this.connecting_input && !isValidConnection(slot_type, in_slot.type)) {
            ctx.globalAlpha = 0.4 * editor_alpha;
          }
          ...
          if (slot_type == 'array') {
            slot_shape = LGraphStyles.GRID_SHAPE;
          }
          ...
          if (
            ...
            ctx.rect(pos[0] + 2, pos[1] + 2, 2, 2);
            doStroke = false;
          } else if (low_quality) ctx.rect(pos[0] - 4, pos[1] - 4, 8, 8);
          else ctx.arc(pos[0], pos[1], 4, 0, Math.PI * 2);
      ...
      if (input_slot) {
        ...
        } else if (slot.shape === LGraphStyles.ARROW_SHAPE) {
          ctx.moveTo(x + 8, y);
          ctx.lineTo(x + -4, y - 4);
          ctx.lineTo(x + -4, y + 4);
          ctx.closePath();
  ...
  drawNodeShape(node, ctx, size, fgcolor, bgcolor, selected, mouse_over) {
    ...
    const shape = node._shape || node.constructor.shape || LGraphStyles.ROUND_SHAPE;

    const { title_mode } = node.constructor;

    let render_title = true;
    if (title_mode == LGraphStyles.TRANSPARENT_TITLE || title_mode == LGraphStyles.NO_TITLE) {
      render_title = false;
    } else if (title_mode == LGraphStyles.AUTOHIDE_TITLE && mouse_over) {
      render_title = true;
    }
    ...
    {
      ctx.beginPath();
      if (shape == LGraphStyles.BOX_SHAPE || low_quality) {
        ctx.fillRect(area[0], area[1], area[2], area[3]);
      } else if (
        shape == LGraphStyles.ROUND_SHAPE
                    || shape == LGraphStyles.CARD_SHAPE
      ) {
        ctx.roundRect(
          area[0],
          area[1],
          area[2],
          area[3],
          shape == LGraphStyles.CARD_SHAPE ? [this.round_radius, this.round_radius, 0, 0] : [this.round_radius],
        );
      } else if (shape == LGraphStyles.CIRCLE_SHAPE) {
        ctx.arc(
          size[0] * 0.5,
    ...
    if (render_title || title_mode == LGraphStyles.TRANSPARENT_TITLE) {
      // title bar
      if (node.onDrawTitleBar) {
        node.onDrawTitleBar(ctx, title_height, size, this.ds.scale, fgcolor);
      } else if (
        title_mode != LGraphStyles.TRANSPARENT_TITLE
        ...
        } else if (shape == LGraphStyles.ROUND_SHAPE || shape == LGraphStyles.CARD_SHAPE) {
          ctx.roundRect(
            0,
            -title_height,
            size[0] + 1,
      ...
      if (node.onDrawTitleBox) {
        ...
        shape == LGraphStyles.ROUND_SHAPE
                    || shape == LGraphStyles.CIRCLE_SHAPE
                    || shape == LGraphStyles.CARD_SHAPE
      ) {
        if (low_quality) {
          ctx.fillStyle = 'black';
      ...
      if (!node.flags.collapsed && node.subgraph && !node.skip_subgraph_button) {
        ...
        const over = isInsideRectangle(this.graph_mouse[0] - node.pos[0], this.graph_mouse[1] - node.pos[1], x + 2, -w + 2, w - 4, w - 4);
        ctx.fillStyle = over ? '#888' : '#555';
        if (shape == LGraphStyles.BOX_SHAPE || low_quality) ctx.fillRect(x + 2, -w + 2, w - 4, w - 4);
        else {
          ctx.beginPath();
    ...
    if (selected) {
      ...
      if (title_mode == LGraphStyles.TRANSPARENT_TITLE) {
        area[1] -= title_height;
        area[3] += title_height;
      }
      ctx.lineWidth = 1;
      ...
      if (shape == LGraphStyles.BOX_SHAPE) {
        ...
        shape == LGraphStyles.ROUND_SHAPE
                    || (shape == LGraphStyles.CARD_SHAPE && node.flags.collapsed)
      ) {
        ctx.roundRect(
          -6 + area[0],
          -6 + area[1],
      ...
      } else if (shape == LGraphStyles.CARD_SHAPE) {
        ctx.roundRect(
          -6 + area[0],
          -6 + area[1],
          12 + area[2],
  ...
  renderLink(
    ...
    for (var i = 0; i < num_sublines; i += 1) {
      ...
      if (this.links_render_mode == LGraphStyles.SPLINE_LINK) {
      ...
      } else if (this.links_render_mode == LGraphStyles.LINEAR_LINK) {
        ctx.moveTo(a[0], a[1] + offsety);
        var start_offset_x = 0;
        var start_offset_y = 0;
        var end_offset_x = 0;
      ...
      } else if (this.links_render_mode == LGraphStyles.STRAIGHT_LINK) {
        ctx.moveTo(a[0], a[1]);
        let start_x = a[0];
        let start_y = a[1];
        let end_x = b[0];
  ...
  processNodeWidgets(node, pos, event, active_widget) {
    ...
    for (let i = 0; i < node.widgets.length; ++i) {
      ...
      switch (w.type) {
        case 'button':
          if (event.type === `${PointerSettings.pointerevents_method}down`) {
            if (w.callback) {
              setTimeout(() => {
        ...
        case 'combo':
          ...
          if (allow_scroll && event.type == `${PointerSettings.pointerevents_method}move` && w.type == 'number') {
          ...
          } else if (event.type == `${PointerSettings.pointerevents_method}down`) {
          ...
          else if (event.type == `${PointerSettings.pointerevents_method}up` && w.type == 'number') {
            var delta = x < 40 ? -1 : x > widget_width - 40 ? 1 : 0;
  ...
  static onMenuAdd(node, options, e, prev_menu, callback) {
    ...
    function inner_onMenuAdded(base_category, prev_menu) {
      const categories = getNodeTypesCategories(canvas.filter || graph.filter).filter((category) => category.startsWith(base_category));
      const entries = [];
      ...
      nodes.map((node) => {
        ...
        const entry = {
          ...
          callback(value, event, mouseEvent, contextMenu) {
            ...
            const node = createNode(value.value);
            if (node) {
  ...
  showLinkMenu(link, e) {
    ...
    function inner_clicked(v, options, e) {
      switch (v) {
        ...
        default:
          /*
            var nodeCreated = createDefaultNodeForSlot({
              nodeFrom: node_left
            ...
            if(nodeCreated)
              console.log("new node in beetween "+v+" created");
          */
      }
    }
  ...
  createDefaultNodeForSlot(optPass) { // addNodeMenu for connection
    ...
    const slotTypesDefault = isFrom ? LGraphNodeRegistry.slot_types_default_out : LGraphNodeRegistry.slot_types_default_in;
    if (slotTypesDefault && slotTypesDefault[fromSlotType]) {
      ...
      if (nodeNewType) {
        ...
        const newNode = createNode(nodeNewType);
        if (newNode) {
  ...
  showSearchBox(event, options) {
    ...
    if (options.do_type_filter) {
      if (selIn) {
        var aSlots = LGraphNodeRegistry.slot_types_in;
      ...
      if (selOut) {
        var aSlots = LGraphNodeRegistry.slot_types_out;
        var nSlots = aSlots.length; // this for object :: Object.keys(aSlots).length;
    ...
    function select(name) {
      if (name) {
        if (that.onSearchBoxSelection) {
          ...
          const extra = LGraphNodeRegistry.searchbox_extras[name.toLowerCase()];
          if (extra) {
            name = extra.type;
          }
    ...
    function refreshHelper() {
      ...
      if (that.onSearchBox) {
        ...
        // extras
        for (var i in LGraphNodeRegistry.searchbox_extras) {
          const extra = LGraphNodeRegistry.searchbox_extras[i];
          if ((!options.show_all_if_empty || str) && extra.desc.toLowerCase().indexOf(str) === -1) {
            continue;
          }
          const ctor = LGraphNodeRegistry.registered_node_types[extra.type];
        ...
        if (Array.prototype.filter) { // filter supported
          const keys = Object.keys(LGraphNodeRegistry.registered_node_types); // types
        ...
        function inner_test_filter(type, optsIn) {
          ...
          const ctor = LGraphNodeRegistry.registered_node_types[type];
          ...
          if (options.do_type_filter && !opts.skipFilter) {
            ...
            if (sIn && sV) {
              ...
              if (LGraphNodeRegistry.registered_slot_in_types[sV] && LGraphNodeRegistry.registered_slot_in_types[sV].nodes) { // type is stored
                console.verbose('[showSearchBox]', `check ${sType} in ${LGraphNodeRegistry.registered_slot_in_types[sV].nodes}`);
                var doesInc = LGraphNodeRegistry.registered_slot_in_types[sV].nodes.includes(sType);
                if (doesInc !== false) {
            ...
            if (sOut && sV) {
              ...
              if (LGraphNodeRegistry.registered_slot_out_types[sV] && LGraphNodeRegistry.registered_slot_out_types[sV].nodes) { // type is stored
                console.verbose('[showSearchBox]', `check ${sType} in ${LGraphNodeRegistry.registered_slot_out_types[sV].nodes}`);
                var doesInc = LGraphNodeRegistry.registered_slot_out_types[sV].nodes.includes(sType);
                if (doesInc !== false) {
  ...
  showShowGraphOptionsPanel(refOpts, obEv, refMenu, refMenu2) {
    ...
    function inner_refresh() {
      ...
      const aProps = undefined; // LiteGraph.availableCanvasOptions;
      aProps.sort();
  ...
  static onMenuNodeShapes(value, options, e, menu, node) {
    ...
    new ContextMenu(LGraphStyles.VALID_SHAPES, {
      event: e,
      callback: inner_clicked,
      parentMenu: menu,
      node,
  ...
  static onMenuNodeToSubgraph(value, options, e, menu, node) {
    ...
    const subgraph_node = createNode('graph/subgraph');
  ...
  static node_colors = node_colors;
}
