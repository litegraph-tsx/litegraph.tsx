...
class GamepadInput {
  ...
  onExecute() {
    ...
    if (gamepad) {
      this._left_axis[0] = Math.abs(gamepad.xbox.axes.lx) > threshold
        ? gamepad.xbox.axes.lx
        : 0;
      this._left_axis[1] = Math.abs(gamepad.xbox.axes.ly) > threshold
        ? gamepad.xbox.axes.ly
        : 0;
      this._right_axis[0] = Math.abs(gamepad.xbox.axes.rx) > threshold
        ? gamepad.xbox.axes.rx
        : 0;
      this._right_axis[1] = Math.abs(gamepad.xbox.axes.ry) > threshold
        ? gamepad.xbox.axes.ry
        : 0;
      this._triggers[0] = Math.abs(gamepad.xbox.axes.ltrigger) > threshold
        ? gamepad.xbox.axes.ltrigger
        : 0;
      this._triggers[1] = Math.abs(gamepad.xbox.axes.rtrigger) > threshold
    ...
    for (let i = this.properties.gamepad_index; i < 4; i++) {
      ...
      for (let j = 0; j < gamepad.buttons.length; j++) {
        ...
        if (j < 12) {
          ...
          // mapping of XBOX
          switch (j) {
            // I use a switch to ensure that a player with another gamepad could play
            case 12:
